	...
	<div id="body_area">
		...
		<div class="midarea">
			...
			<div class="body_textarea">
				<div align="justify">
				  <p>แจ้งเตือนรายชื่อพนักงานที่เหลืออีก 7 วันครบกำหนดการสอบ</p>
				  <p>&nbsp;</p>
				  <form id="form1" name="form1" method="get" action="main-warn-7day.php">
                    ...
                    <div><a href="admin-pdf-7.php?sup=<?php echo $_GET[sel_product]?>" target="_blank"><img src="images\PDF_Logo.png" align="right" border="0" width="64" height="64"></a><a href="admin-exl-7.php?sup=<?php echo $_GET[sel_product]?>" target="_blank"><img src="images\Excel2007Logo.png" align="right" border="0" width="60" height="64"></a></div>
			...
			<div class="body_textarea">
				...
				<?php
					...
					@$num_rows=mysql_num_rows($result);
						if(!$num_rows){
							echo "<meta http-equiv='refresh' content=0;URL=main-warn-7day.php>";
						}
					$p=1;
					$t=0;
					...
					if($n<=0){
						...
						for($i=0;$i<$n;$i++){
							echo"<tr><td>".($i+1)."</td><td>{$a[$b[$i]][0]}</td><td>{$a[$b[$i]][1]}</td><td>{$a[$b[$i]][2]}</td><td>{$a[$b[$i]][3]}</td><td>{$a[$b[$i]][4]}</td></tr>";
						}
					}
					echo "<caption align=\"bottom\">result : ".($n-1)." </caption>";
				?>
				</table>
				</div>
			</div>
...
<?php
	...
	echo"<center><h4>ระบบจะพาท่านกลับสู่หน้าหลัก ภายใน 3 วินาที</h4></center>".$_SESSION['MM_UserRight'];

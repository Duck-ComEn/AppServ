...
// This page prints a particular instance of quiz
    ...
    if ($attempts) {
		...
		//Add employees E/N and name
		//echo "<br>";
		echo "<center><h2>E/N : ".$USER->idnumber."&nbsp;&nbsp;".$USER->firstname." ".$USER->lastname."</h2></center>";
		//echo $USER->username;
		//echo $COURSE->id;
		...
		//Column name for percentcolumn
		$percentcolumn = $someoptions->scores && $quiz->grade && $quiz->sumgrades;
        //$markcolumn = $gradecolumn && ($quiz->grade != $quiz->sumgrades);
        //$overallstats = $alloptions->scores;
		//TUM
		...
		//Header percentcolumn
        if ($percentcolumn) {
            $table->head[] = "Percent %";
            $table->align[] = 'center';
            $table->size[] = '';
        }
		//End Header percentcolumn
        if ($feedbackcolumn) {
...
//calculate percent of each users score
$usergrade=$attempt->sumgrades;
$sumgrade=$quiz->grade;
$resultpercent=round($usergrade/$sumgrade,2)*100;
//end calcuate
            // Ouside the if because we may be showing feedback but not grades.
			$attemptgrade_old=quiz_rescale_grade($attempt->sumgrades, $quiz);//create new variable
            $attemptgrade = quiz_rescale_grade($attempt->sumgrades, $quiz);
			$resultpercent_Print = $resultpercent." %";//create new variable for percentcolumn
            ...
            if ($percentcolumn) {
                if ($attemptoptions->scores && $attempt->timefinish > 0) {
                    $formattedpercent = $resultpercent_Print;
                    // highlight the highest grade if appropriate
                    /* if ($overallstats && $numattempts > 1 && !is_null($mygrade) && $attemptgrade == $mygrade && $quiz->grademethod == QUIZ_GRADEHIGHEST) {
                        $table->rowclass[$attempt->attempt] = 'bestrow';
                    } */

                    $row[] = make_review_link($formattedpercent, $quiz, $attempt, $context);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($feedbackcolumn && $attempt->timefinish > 0) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = quiz_feedback_for_grade($attemptgrade_old, $quiz->id);
                } else {
                    $row[] = '';
                }
            }			

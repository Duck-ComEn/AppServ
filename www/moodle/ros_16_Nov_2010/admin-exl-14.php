...
header('Content-Disposition: attachment; filename="report-warnning-all.xls"');
					...
					union ALL
						...
						where mdl_user.institution = '".$_GET[sup]."'
						ORDER BY 7");
							}
...
echo "<br><br><br><table border=1><caption align=top>Report Recertification Warnning (14 Days)</caption>";
echo "<tr><td width=58 bgcolor=#cccccc align=center>E/N</td><td width=200 bgcolor=#cccccc>Name</td><td width=330 bgcolor=#cccccc>Course</td><td width=90 bgcolor=#cccccc align=center>Time</td><td width=100 bgcolor=#cccccc align=center>Remain Time</td></tr>";
for($i=0;$i<$n;$i++){
echo "<tr><td width=58>{$a[$b[$i]][0]}</td><td width=200>{$a[$b[$i]][1]}</td><td width=330>{$a[$b[$i]][2]}</td><td width=90 align=center>{$a[$b[$i]][3]}</td><td width=100 align=center>{$a[$b[$i]][4]}</td></tr>";
}
echo "</table>";
}
?>